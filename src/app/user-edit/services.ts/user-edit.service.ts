...
export class UserEditService {
  ...
  putAvatar(formData): Observable<object> {
    ...
    return this.httpClient.post('https://api.imgur.com/3/image', formData, options).pipe(map((res: any) => {
      const avatarPath = res.data.link; //Imgur response
      this.httpClient.put(`${environment.apiUrl}/account/avatar`, { 'avatarPath': avatarPath }, this.httpOptions).subscribe((res) => {
        return res;
      });
      return res;
    }));
